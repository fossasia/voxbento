...
async def main() -> None:
    ...
    async with get_session() as s:
        ...
        check('Uppercase slug normalised to lowercase', caps_event.slug == 'has-caps',
              f'got {caps_event.slug}')
    ...
    async with get_session() as s:
        foss_keynote = await create_room(
            s, event_id=fossasia.id, display_name='Keynote Stage',
    ...
    for ev_id, room_id, lang_code, lang_name in languages:
        async with get_session() as s:
            booth = await create_booth(
                s, event_id=ev_id, room_id=room_id,
    ...
    async with get_session() as s:
        ...
        check('mediamtx_path = pycon2026/en', booth_en.mediamtx_path == 'pycon2026/en',
              f'got {booth_en.mediamtx_path}')

        foss_zh_id = booth_ids[f'{fossasia.id}-zh']
        booth_zh = await get_booth_by_id(s, foss_zh_id)
        check('mediamtx_path = fossasia2026/zh', booth_zh.mediamtx_path == 'fossasia2026/zh',
              f'got {booth_zh.mediamtx_path}')

        dev_cs_id = booth_ids[f'{devconf.id}-cs']
        booth_cs = await get_booth_by_id(s, dev_cs_id)
        check('mediamtx_path = devconf-cz-2026/cs', booth_cs.mediamtx_path == 'devconf-cz-2026/cs',
    ...
    async with get_session() as s:
        try:
            await create_booth(
                s, event_id=pycon.id, room_id=pycon_main.id,
    ...
    async with get_session() as s:
        ...
        check('Token booth.event.slug = pycon2026', tok.booth.event.slug == 'pycon2026',
              f'got {tok.booth.event.slug}')
    ...
    async with get_session() as s:
        throwaway_booth = await create_booth(
            s, event_id=devconf.id, room_id=dev_main.id,
    ...
    async with get_session() as s:
        ...
        test_booth = await create_booth(
            s, event_id=devconf.id, room_id=test_room.id,
            language_code='pl', language_name='Polish',
        )
        test_tok = await create_invite_token(
            s, booth_id=test_booth.id, role='interpreter',
        )
        tr_id = test_room.id
